...
if(strcmp(modelname,'ssc_airvtol_elec'))
    if ~exist('h3_ssc_airvtol_elec', 'var') || ...
            ~isgraphics(h3_ssc_airvtol_elec, 'figure')
        h3_ssc_airvtol_elec = figure('Name', 'ssc_airvtol_elec');
    end
    figure(h3_ssc_airvtol_elec)
    clf(h3_ssc_airvtol_elec)
    temp_clr_ind = 1;
    temp_plotsym = 'o-';
else
    if ~exist('h3_ssc_airvtol_elec_hybrid', 'var') || ...
            ~isgraphics(h3_ssc_airvtol_elec_hybrid, 'figure')
        h3_ssc_airvtol_elec_hybrid = figure('Name', 'ssc_airvtol_elec_hybrid');
    end
    figure(h3_ssc_airvtol_elec_hybrid)
    clf(h3_ssc_airvtol_elec_hybrid)
	temp_clr_ind = 2;
    temp_plotsym = '*-';
end
