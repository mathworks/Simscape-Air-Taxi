...
for bc_i=1:length(battery_capacity_set)
    ...
    % Loop over payload
    ssc_airvtol_elec_sweep_payload_xllink
...
if(strcmp(modelname,'ssc_airvtol_elec'))
    if ~exist('h8_ssc_airvtol_elec', 'var') || ...
            ~isgraphics(h8_ssc_airvtol_elec, 'figure')
        h8_ssc_airvtol_elec = figure('Name', 'ssc_airvtol_elec');
    end
    figure(h8_ssc_airvtol_elec)
    clf(h8_ssc_airvtol_elec)
    temp_annotation_str = 'Pure Electric';
else
    if ~exist('h8_ssc_airvtol_elec_hybrid', 'var') || ...
            ~isgraphics(h8_ssc_airvtol_elec_hybrid, 'figure')
        h8_ssc_airvtol_elec_hybrid = figure('Name', 'ssc_airvtol_elec_hybrid');
    end
    figure(h8_ssc_airvtol_elec_hybrid)
