% Code to sweep payload mass for a single electric aircraft VTOL example and
% plot results.
%
...
if(strcmp(modelname,'ssc_airvtol_elec'))
    if ~exist('h4_ssc_airvtol_elec', 'var') || ...
            ~isgraphics(h4_ssc_airvtol_elec, 'figure')
        h4_ssc_airvtol_elec = figure('Name', 'ssc_airvtol_elec');
    end
    figure(h4_ssc_airvtol_elec)
    clf(h4_ssc_airvtol_elec)
    temp_clr_ind = 1;
    temp_plotsym = 'o-';
else
    if ~exist('h4_ssc_airvtol_elec_hybrid', 'var') || ...
            ~isgraphics(h4_ssc_airvtol_elec_hybrid, 'figure')
        h4_ssc_airvtol_elec_hybrid = figure('Name', 'ssc_airvtol_elec_hybrid');
    end
    figure(h4_ssc_airvtol_elec_hybrid)
